function out = compute_fittingStats(y, target, N, K, SS_tot, optimOut)

    % y        - fit
    % target   - experimental data
    % N        - number of data points used to fit the experimental curve
    % K        - number of free parameters   
    % SS_tot   - total sum of square of the experimental data        
    ...
    if strcmp(optimOut, 'optim')
        % Value to be minized
        out = SS_err_norm;       
        
    elseif strcmp(optimOut, 'spectrum')        
        out.R2 = R2 + 1;
        out.AIC = AIC;        
        out.AICc = AICc;
        out.ABIC = ABIC;
