function drawGrid(handles, x, y)

    % wrapper function to draw a nicer grid using a 3rd party subfunction "gridx"
    % Petteri Teikari, petteri.teikari@gmail.com
    
    % http://www.mathworks.com/matlabcentral/fileexchange/9973 
    % http://www.mathworks.com/matlabcentral/fileexchange/28244-gridxy2
    
    % GRIDXY(X) plots vertical grid lines at the positions specified by X. 
    % GRIDXY(X,Y) also plots horizontal grid lines at the positions specified by Y. 
    % When X or Y is empty no vertical or horizontal lines are plotted.
    
    if nargin == 0 % no arguments given
        minorColor      = [.50 .50 .50];
        majorColor      = [.75 .75 .75];
        lineWdtMajor    = 1;
        lineStyMajor    = ':';
        ...
        x = get(gca,'XTick'); 
        y = get(gca,'YTick'); % get major grid automatically from Ticks
        xInterv = (x(2) - x(1))/2;
            x2 = (min(x)-xInterv) : xInterv : (max(x)+xInterv); % minor x
        yInterv = (y(2) - y(1))/2;
            y2 = (min(y)-yInterv) : yInterv : (max(y)+yInterv); % minor x
        
    elseif nargin == 1 % only handles specified                
        x = get(gca,'XTick'); 
        y = get(gca,'YTick'); % get major grid automatically from Ticks
        xInterv = (x(2) - x(1))/2;
            x2 = (min(x)-xInterv) : xInterv : (max(x)+xInterv); % minor x
    ...
    if nargin ~= 0        
        minorColor      = handles.grid_minorColor;
        majorColor      = handles.grid_majorColor;
        lineWdtMajor    = handles.grid_lineWidth_major;
        lineStyMajor    = handles.grid_lineStyle_major;
        lineWdtMinor    = handles.grid_lineWidth_minor;
        lineStyMinor    = handles.grid_lineStyle_minor;
    end

    % Major X-grid        
    gridxy(x,[], 'Color',majorColor, 'linewidth', lineWdtMajor, 'Linestyle', lineStyMajor);

    % Minor x-grid        
    gridxy(x2,[],'Color',minorColor,'linewidth', lineWdtMinor,'Linestyle', lineStyMinor);

    % Major Y-grid        
    gridxy([],y,'Color',majorColor,'linewidth', lineWdtMajor,'Linestyle', lineStyMajor);

    % Minor y-grid        
    gridxy([],y2,'Color',minorColor,'linewidth', lineWdtMinor,'Linestyle', lineStyMinor);
